...
for (let i = 0; i < productContainers.length; i++) {
  // Show price when product is hovered
  productContainers[i].addEventListener('mouseover', () => {
    price[i].style.display = 'inline';
    price[i].classList.add('animate-price');
  });
  // Hide price when product is  not hovered
  productContainers[i].addEventListener('mouseout', () => {
    price[i].style.display = 'none';
    price[i].classList.remove('animate-price');
  });
}
